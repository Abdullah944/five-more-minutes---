extends Object
class_name StoreTabThemes

## Store tab: UI theme catalog rows (unlock / equip).
...
static func populate(item_list: VBoxContainer, kit: Variant, host: Control) -> void:
	...
	for theme_id: String in MetaProgression.THEME_CATALOG:
		var info: Dictionary = MetaProgression.THEME_CATALOG[theme_id]
		var owned := theme_id in MetaProgression.unlocked_themes
		var selected := theme_id == MetaProgression.selected_theme
		...
		if selected:
			...
			action_btn.pressed.connect(host.equip_theme.bind(theme_id))
		else:
			...
			if can_afford:
				...
				action_btn.pressed.connect(host.buy_theme.bind(theme_id))
			else:
