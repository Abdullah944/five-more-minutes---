extends Object
class_name StoreTabPajamas

## Store tab: pajama catalog rows (unlock / equip).

const PAJAMA_ICON := "[PJ]"
...
static func populate(item_list: VBoxContainer, kit: Variant, host: Control) -> void:
	...
	for pj_id: String in MetaProgression.PAJAMA_CATALOG:
		var info: Dictionary = MetaProgression.PAJAMA_CATALOG[pj_id]
		var owned := pj_id in MetaProgression.unlocked_pajamas
		var selected := pj_id == MetaProgression.selected_pajama
		...
		if selected:
			...
			action_btn.pressed.connect(host.equip_pajama.bind(pj_id))
		else:
			...
			if can_afford:
				...
				action_btn.pressed.connect(host.buy_pajama.bind(pj_id))
			else:
